...
for d in unique_digits:
    serialisers.append(Numpy(f"serialiser_{d}"))

# Evaluate model on numpy dataset
start_time = perf_counter() 

for count, digit_left in enumerate(unique_digits):
    #reset gpu memory
    print(f"speaker {digit_left} of {len(unique_digits)}")

    train_spikes, train_labels = [],[]
    eval_spikes, eval_labels = [],[]
    for i in np.where(y_train != digit_left)[0]:
        train_spikes.append(x_train_spikes[i])
        train_labels.append(z_train[i])
    for i in np.where(y_train == digit_left)[0]:
        eval_spikes.append(x_train_spikes[i])
        eval_labels.append(z_train[i])
    ...
    with compiled_net:
        ...
        callbacks = [Checkpoint(serialisers[count]), 
                     CSVTrainLog(f"train_output_{digit_left}.csv", 
                                 output,
                                 False)]
        ...
        for e in trange(params["NUM_EPOCH"]):
            ...
            CSVValidationLog(f"validation_output_{digit_left}.csv", e, metrics_val, output)
